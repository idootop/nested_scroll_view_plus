...
class NestedScrollViewStatePlus extends State<OriginalNestedScrollView> {
  final OriginalOverlapAbsorberHandle _absorberHandle =
...
class _OriginalInheritedNestedScrollView extends InheritedWidget {
  ...
  @override
  bool updateShouldNotify(_OriginalInheritedNestedScrollView old) =>
      state != old.state;
}
...
typedef _OriginalNestedScrollActivityGetter = ScrollActivity Function(
    _OriginalNestedScrollPosition position);

class _OriginalNestedScrollCoordinator
  ...
  bool get canScrollBody {
    final _OriginalNestedScrollPosition? outer = _outerPosition;
    if (outer == null) {
      return true;
    }
  ...
  void beginActivity(ScrollActivity newOuterActivity,
      _OriginalNestedScrollActivityGetter innerActivityGetter) {
  ...
  ScrollActivity createInnerBallisticScrollActivity(
      _OriginalNestedScrollPosition position, double velocity) {
  ...
  void pointerScroll(double delta) {
    ...
    if (_innerPositions.isEmpty) {
      ...
      if (outerDelta.abs() > precisionErrorTolerance) {
        ...
        if (innerDelta != 0.0) {
          for (final _OriginalNestedScrollPosition position
              in _innerPositions) {
  ...
  Drag drag(DragStartDetails details, VoidCallback dragCancelCallback) {
    ...
    beginActivity(
      ...
      (_OriginalNestedScrollPosition position) =>
          DragScrollActivity(position, drag),
    );
  ...
  void applyUserOffset(double delta) {
    ...
    if (_innerPositions.isEmpty) {
      ...
      if (innerDelta != 0.0) {
        ...
        final List<_OriginalNestedScrollPosition> innerPositions =
            _innerPositions.toList();
        for (final _OriginalNestedScrollPosition position in innerPositions) {
...
class _OriginalNestedScrollPosition extends ScrollPosition
...
class _OriginalNestedInnerBallisticScrollActivity
...
class _OriginalNestedOuterBallisticScrollActivity
    extends BallisticScrollActivity {
...
class OriginalSliverOverlapAbsorber extends SingleChildRenderObjectWidget {
  ...
  void updateRenderObject(
      BuildContext context, OriginalRenderSliverOverlapAbsorber renderObject) {
...
class OriginalRenderSliverOverlapAbsorber extends RenderSliver
...
class OriginalSliverOverlapInjector extends SingleChildRenderObjectWidget {
  ...
  @override
  void updateRenderObject(
      BuildContext context, OriginalRenderSliverOverlapInjector renderObject) {
    renderObject.handle = handle;
  }
